...
def clavescandidatas(R,F):
  ...
  for item in partes_list:
    # La clave candidata es el mínimo subconjunto de una superclave. Así que si F1 es clave, F1F2 no.
    # Cada clave candidata es una superclave pero cada superclaveclave no es una clave candidata.
    if (max_len != 0 and len(item) > max_len):
      #print("Es una clave más larga que la otra: {}, no la agrego",item)
      continue

    # Pruebo si el item de la lista si es una superclave.
    itemplus = ca(F,item)
    if (itemplus == R):
      #print("Forma parte de un conjunto así que lo agregamos, clave: ",item)
